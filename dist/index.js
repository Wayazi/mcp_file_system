...
function auditLog(operation, filePath, success, error) {
    if (CONFIG.auditLog) {
...
server.tool('read_multiple_files', { paths: z.array(z.string()).describe('Array of file paths to read') }, async ({ paths }) => {
    ...
    const output = results
        ...
        if (result.status === 'fulfilled') {
            return `${result.value.path}: ${result.value.success ? result.value.content : `Error: ${result.value.error}`}`;
        }
...
// Tool: Delete file
server.tool('delete_file', { path: z.string().describe('File path to delete') }, async ({ path: filePath }) => {
    try {
        const validPath = validatePath(filePath);
